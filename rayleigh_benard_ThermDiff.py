...
Options:
    ...
    --run_time_buoy=<run_time_bouy>   Run time, in buoyancy times [default: 10000.]
...
def Rayleigh_Benard(Rayleigh=1e6, Prandtl=1, nz=64, nx=None, aspect=2,
                    fixed_flux=False, fixed_T=True,
                    viscous_heating=False, restart=None,
                    run_time=150., run_time_buoyancy=20000., run_time_iter=np.inf,
    ...
    problem.add_equation("dt(T) - (P / phi) * (dx(dx(T * phi)) + dz(Tz + phi)) + w*T0_z    = -(u*dx(T) + w*Tz)  - visc_heat")
